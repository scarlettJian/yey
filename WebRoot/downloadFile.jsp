...
function top2() {//首页
	form3.action="<%=basePath %>downloadFile.jsp?page=1";
	form3.submit(); 
}
function last2() {//尾页
	 if(form3.pageCount.value==0){//如果总页数为0，那么最后一页为1，也就是第一页，而不是第0页
			...
			form3.action="<%=basePath%>downloadFile.jsp?page="+form3.pageCount.value;
	    	form3.submit();
		}
}
function pre2(){//上一页
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>downloadFile.jsp?page="+(page-1);
    form3.submit();
  }
}
function next2(){//下一页
...
<div class="tom"><div class="totitle"><span> </span></div></div>  
   ...
   <table width="100%" border="0" align="left" cellpadding="4" cellspacing="0" class="rtable">
	  <tbody>
	  <tr class="tr1">
			<td class="rldate daslist">文件下载中心</td>
...
%>
	     <tr class="tr<%=(i)%2%>"> 
            <td class="rltitle dotlist"><a href="<%=basePath %>downloadFileInfo.jsp?id=<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(1).toString() %></a></td> 
